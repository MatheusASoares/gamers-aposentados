...
export function GameCard({ game }: GameCardProps) {
    ...
    const getBadgeVariant = (status: string) => {
        switch (status) {
            ...
            case "COMPLETED":
                return "secondary"; // Cinza claro/Verde (se customizar)
            case "DROPPED":
    ...
    return (
        ...
        <Card className="transition-shadow hover:shadow-md">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="truncate pr-2 text-lg font-bold" title={game.title}>
                    {game.title}
                </CardTitle>
                <Badge variant={getBadgeVariant(game.status)}>{game.status}</Badge>
            </CardHeader>
            ...
            <CardContent>
                <div className="text-muted-foreground space-y-2 text-sm">
                    <div className="flex justify-between">
                        ...
                        <span className="text-foreground font-medium">{game.nominatedBy}</span>
                    </div>
