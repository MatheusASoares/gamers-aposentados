...
export async function PUT(request: Request) {
  try {
    ...
    if (action === "draw") {
      ...
      const poolWithEntries = await prisma.pool.findUnique({ where: { id }, include: { entries: true } });
      if (!poolWithEntries) return NextResponse.json({ error: "Pool not found" }, { status: 404 });
