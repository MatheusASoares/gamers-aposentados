...
const mapInputToDb = (input: any) => {
  ...
  if (input.startDate !== undefined) data.start_date = input.startDate ? new Date(input.startDate) : null;
...
export async function POST(request: Request) {
  try {
    ...
    if (!nominatedById) return NextResponse.json({ error: "nominatedById is required" }, { status: 400 });
