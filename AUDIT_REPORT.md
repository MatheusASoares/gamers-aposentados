...
### 1.1 Regra de Bloqueio em `joinQuest` Impede Início de Quests Sorteadas (`SUGGESTED`)
...
- **Gravidade:** **Alta**
- **Causa Raiz:** Quando uma pool é sorteada pelo Randomizer, registros em `game_progress` são criados com status `SUGGESTED`. Quando um jogador clica em *"Entrar na Quest"*, a função `joinQuest` encontra o registro existente e cai no bloco que valida se o jogo já foi dropado:
  ```typescript
...
### 1.2 `NoticeBoardMuralClient` Falha em Múltiplos Usuários e Usuários Novos
...
### 1.3 `executeRoll` Ignora Reativação de Jogos Previamente Dropados
...
### 1.4 Inconsistência de IDs (UUID vs IGDB ID) no Randomizer
...
### 1.5 Falha na Recalculação de XP ao Dropar Quest
...
### 2.1 Server Action Pública Sem Autenticação: `runBackfillXP`
...
### 2.2 Troca de Senha Sem Exigir Senha Atual (`changePassword`)
...
### 2.3 Risco de Apicalypse Query Injection na Integração IGDB
...
export async function changePassword(data: z.infer<typeof ChangePasswordSchema>) {
    ...
    if (!user || !user.password) {
        return { success: false, error: "Usuário não encontrado ou autenticado por provedor social." };
    }
